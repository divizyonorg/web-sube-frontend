...
test.describe('Anasayfa', () => {
  ...
  test('Kredi Uygunluk kartı yüklenir ve skoru gösterir', async ({ page }) => {
    ...
    if (await kurBtn.count() === 0) bug('"K.U.R Raporunu Gör" butonu Kredi Uygunluk kartında bulunamadı');
  });
  ...
  test('Kredi Uygunluk slider/skor değeri DOM\'da mevcut', async ({ page }) => {
    ...
    if (await sliderEl.count() === 0) bug('Kredi Uygunluk slider elementi bulunamadı — API verisi gelmiyor olabilir');
  });
  ...
  test('Kredi Danışmanlığı kartı yüklenir ve butonu görünür', async ({ page }) => {
    ...
    if (await uzmanBtn.count() === 0) bug('"Kredi Uzmanı ile Devam Et" butonu Danışmanlık kartında bulunamadı');
  });
  ...
  test('VIP Paketler kartı içeriği görünür', async ({ page }) => {
    ...
    if (!await vipLink.isVisible()) bug('VIP Paketleri İncele linki DOM\'da var ama görünmüyor — CSS hidden sorunu');
  });
  ...
  test('Popüler Krediler swiper görünür ve slide\'lar yüklü', async ({ page }) => {
    ...
    if (count === 0) bug('Popüler Krediler swiper\'ında hiç slide yok');
    else info(`Popüler Krediler slide sayısı: ${count}`);
  });
  ...
  test('Popüler Krediler kartında Başvur butonu görünür', async ({ page }) => {
    ...
    if (await basvurBtn.count() === 0) bug('Popüler Krediler kartında "Başvur" butonu bulunamadı');
  });
  ...
  test('Avantajlı Kredi Kartları swiper görünür', async ({ page }) => {
    ...
    if (count === 0) bug('Avantajlı Kredi Kartları swiper\'ında hiç slide yok');
    else info(`Kredi Kartları slide sayısı: ${count}`);
  });
  ...
  test('Kampanyalar verisi yükleniyor mu?', async ({ page }) => {
    ...
    if (slideCount === 0) bug('Kampanyalar bölümünde hiç veri yok — API\'den kampanya gelmiyor');
    ...
    if (isLocked) bug('Kampanyalar swiper nav butonları swiper-button-lock ile kilitli — yeterli slide yok');
  });
