...
test.describe('Kredi Raporları', () => {
  ...
  test('özet kartları görünür (Toplam, Hazır, İşleniyor)', async ({ page }) => {
    ...
    for (const [name, locator] of [
      ...
      if (await locator.count() === 0) bug(`"${name}" özet kartı sayfada bulunamadı`);
    }
  });
  ...
  test('rapor listesi yükleniyor', async ({ page }) => {
    ...
    if (count === 0) bug('Rapor listesi boş — bu kullanıcıya ait rapor olmayabilir veya API verisi gelmiyor');
  });
  ...
  test('rapor varsa görüntüleme ve indirme butonları görünür', async ({ page }) => {
    ...
    if (await viewBtn.count() === 0) {
      bug('"Raporu Gör" butonu yok — listede rapor olmayabilir');
    } else {
