...
test.describe('Register Sayfası', () => {
  ...
  test('sayfa yüklenir ve form görünür', async ({ page }) => {
    ...
    if (finalUrl.includes('/login') || finalUrl.includes('/Login'))
      bug('/register sayfası login\'e yönlendiriyor — sayfa korumalı veya mevcut değil');
    const hasHeading = await page.locator('h1, h2').first().isVisible({ timeout: 10_000 }).catch(() => false);
    if (!hasHeading) bug('/register sayfası yüklenmedi veya başlık görünmüyor');
  });
  ...
  test('telefon doğrula butonu telefon girilmeden devre dışı', async ({ page }) => {
    ...
    if (!await dogrulaBtn.isDisabled()) bug('Telefon girilmeden Doğrula butonu aktif — validasyon çalışmıyor');
  });
  ...
  test('telefon girilince Doğrula butonu aktifleşir', async ({ page }) => {
    ...
    const dogrulaBtn = page.locator('button:has-text("Doğrula")').first();
    if (!await dogrulaBtn.isEnabled()) bug('Telefon girilince Doğrula butonu aktifleşmiyor');
  });
  ...
  test('Üye Ol butonu telefon doğrulanmadan devre dışı', async ({ page }) => {
    ...
    if (!await uyeOlBtn.isDisabled()) bug('Telefon doğrulanmadan Üye Ol butonu aktif');
  });
  ...
  test('onay kutucukları tıklanabilir', async ({ page }) => {
    ...
    if (!await checkboxes.first().isChecked()) bug('Onay kutucuğuna tıklayınca işaretlenmiyor');
  });
  ...
  test('Açık Rıza Metni modalı açılır', async ({ page }) => {
    ...
    if (!await modal.isVisible().catch(() => false)) bug('Açık Rıza Metni linkine tıklanınca modal açılmıyor');
  });
  ...
  test('Aydınlatma Metni modalı açılır', async ({ page }) => {
    ...
    if (!await modal.isVisible().catch(() => false)) bug('Aydınlatma Metni linkine tıklanınca modal açılmıyor');
  });
