...
test.describe('Canlı Destek', () => {
  ...
  test('destek ekibi başlığı ve durum göstergesi görünür', async ({ page }) => {
    ...
    if (await page.locator('text=Destek Ekibi').count() === 0)
  ...
  test('mesaj input alanı görünür ve yazılabilir', async ({ page }) => {
    ...
    if (await msgInput.inputValue() !== 'Test mesajı')
      bug('Mesaj input alanına yazılamıyor');
  });
  ...
  test('gönder butonu boş inputta devre dışı', async ({ page }) => {
    ...
    await page.locator('input[type="text"], textarea').last().fill('');
    await page.waitForTimeout(200);
    if (!await sendBtn.isDisabled())
      bug('Boş inputta gönder butonu aktif — validasyon eksik');
  });
  ...
  test('mesaj gönderme akışı çalışır', async ({ page }) => {
    ...
    if (await page.locator('text=Merhaba, yardım lazım').count() === 0)
  ...
  test('hızlı cevap butonları görünür ve çalışır', async ({ page }) => {
    ...
    info(`Hızlı cevap buton sayısı: ${count}`);
    if (count === 0) {
    ...
    if (!await page.locator('input[type="text"], textarea').last().inputValue())
      bug('Hızlı cevap butonuna basınca input dolmuyor');
  });
