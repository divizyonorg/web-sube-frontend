...
test.describe('Faturalarım', () => {
  ...
  test('fatura listesi görünür', async ({ page }) => {
    ...
    if (count === 0) bug('Fatura listesi boş — statik demo veri bile görünmüyor olabilir');
  });
  ...
  test('fatura durum badge\'leri görünür (Ödendi/Ödenmedi)', async ({ page }) => {
    ...
    if (!odendiExists && !odenmediExists) bug('Fatura durum badge\'leri (Ödendi/Ödenmedi) sayfada bulunamadı');
  });
  ...
  test('"Şimdi Öde" butonu ödenmemiş faturada görünür', async ({ page }) => {
    ...
    if (await odeBtn.count() === 0) {
      bug('"Şimdi Öde" butonu bulunamadı — ödenmemiş fatura yok veya buton eksik');
    } else {
  ...
  test('görüntüleme ve indirme butonları görünür', async ({ page }) => {
    ...
    info(`Fatura görüntüleme butonu sayısı: ${await viewBtns.count()}`);
  });
