...
test.describe('VIP Danışmanlık Paketleri', () => {
  ...
  test('sayfa yüklenir ve başlık görünür', async ({ page }) => {
    ...
    const heroText = page.locator('text=Krediye Her Zaman Hazır Ol').or(page.locator('text=VIP')).or(page.locator('text=Danışmanlık')).first();
    if (await heroText.count() === 0) bug('VIP sayfası yüklendi ama beklenen içerik bulunamadı');
  });
  ...
  test('"En Popüler" badge görünür', async ({ page }) => {
    await page.locator('h1').first().waitFor({ timeout: 15_000 });
    if (await page.locator('text=En Popüler').first().count() === 0)
  ...
  test('%25 indirim banneri görünür', async ({ page }) => {
    ...
    if (await banner.count() === 0) bug('İndirim banneri sayfada bulunamadı');
  });
